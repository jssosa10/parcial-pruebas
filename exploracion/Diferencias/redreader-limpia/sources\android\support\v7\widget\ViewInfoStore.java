...
class ViewInfoStore {
    ...
    private ItemHolderInfo popFromLayoutStep(ViewHolder vh, int flag) {
        ...
        record.flags &= flag ^ -1;
        if (flag == 4) {
            info2 = record.preInfo;
        } else if (flag == 8) {
            info2 = record.postInfo;
        } else {
    ...
    /* access modifiers changed from: 0000 */
    public void addToAppearedInPreLayoutHolders(ViewHolder holder, ItemHolderInfo info2) {
        InfoRecord record = (InfoRecord) this.mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            this.mLayoutHolderMap.put(holder, record);
    ...
    /* access modifiers changed from: 0000 */
    public void addToPostLayout(ViewHolder holder, ItemHolderInfo info2) {
        InfoRecord record = (InfoRecord) this.mLayoutHolderMap.get(holder);
        if (record == null) {
            record = InfoRecord.obtain();
            this.mLayoutHolderMap.put(holder, record);
        }
        record.postInfo = info2;
        record.flags |= 8;
    }
    ...
    public void removeViewHolder(ViewHolder holder) {
        ...
        InfoRecord info2 = (InfoRecord) this.mLayoutHolderMap.remove(holder);
        if (info2 != null) {
            InfoRecord.recycle(info2);
        }
    }
