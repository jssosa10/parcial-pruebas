...
final class CascadingMenuPopup extends MenuPopup implements MenuPresenter, OnKeyListener, OnDismissListener {
    ...
    public void dismiss() {
        ...
        if (length > 0) {
            ...
            for (int i = length - 1; i >= 0; i--) {
                CascadingMenuInfo info2 = addedMenus[i];
                if (info2.window.isShowing()) {
                    info2.window.dismiss();
                }
            }
        }
    }
    ...
    public void onDismiss() {
        ...
        while (true) {
            ...
            CascadingMenuInfo info2 = (CascadingMenuInfo) this.mShowingMenus.get(i);
            if (!info2.window.isShowing()) {
                dismissedInfo = info2;
                break;
            }
    ...
    public void updateMenuView(boolean cleared) {
        for (CascadingMenuInfo info2 : this.mShowingMenus) {
            toMenuAdapter(info2.getListView().getAdapter()).notifyDataSetChanged();
        }
    }
    ...
    public boolean onSubMenuSelected(SubMenuBuilder subMenu) {
        for (CascadingMenuInfo info2 : this.mShowingMenus) {
            if (subMenu == info2.menu) {
                info2.getListView().requestFocus();
                return true;
            }
        }
    ...
    public void onCloseMenu(MenuBuilder menu, boolean allMenusAreClosing) {
        ...
        if (menuIndex >= 0) {
            ...
            CascadingMenuInfo info2 = (CascadingMenuInfo) this.mShowingMenus.remove(menuIndex);
            info2.menu.removeMenuPresenter(this);
            if (this.mShouldCloseImmediately) {
                info2.window.setExitTransition(null);
                info2.window.setAnimationStyle(0);
            }
            info2.window.dismiss();
            int count = this.mShowingMenus.size();
            if (count > 0) {
