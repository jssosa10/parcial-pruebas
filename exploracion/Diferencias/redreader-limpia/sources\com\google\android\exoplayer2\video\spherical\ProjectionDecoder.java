...
public final class ProjectionDecoder {
    ...
    public static Projection decode(byte[] projectionData, int stereoMode) {
        ...
        try {
            arrayList = isProj(input) ? parseProj(input) : parseMshp(input);
        ...
        switch (arrayList.size()) {
            case 1:
                return new Projection((Mesh) arrayList.get(0), stereoMode);
            case 2:
                return new Projection((Mesh) arrayList.get(0), (Mesh) arrayList.get(1), stereoMode);
            default:
                return null;
        }
    }
    ...
    private static Mesh parseMesh(ParsableByteArray input) {
        ...
        int vertexIndex = 0;
        for (int vertex = 0; vertex < coordinate2; vertex++) {
            int i2 = 0;
            while (i2 < 5) {
        ...
        while (i4 < subMeshCount) {
            ...
            while (counter < triangleIndexCount) {
                ...
                if (index2 < 0 || index2 >= coordinate2) {
                    return null;
                }
            ...
            subMeshes[i4] = new SubMesh(textureId, triangleVertices, textureCoords, drawMode);
            i4++;
