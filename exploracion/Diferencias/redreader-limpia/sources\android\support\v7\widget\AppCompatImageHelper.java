...
public class AppCompatImageHelper {
    ...
    private boolean applyFrameworkTintUsingColorFilter(@NonNull Drawable imageSource) {
        ...
        TintInfo info2 = this.mTmpInfo;
        info2.clear();
        ColorStateList tintList = ImageViewCompat.getImageTintList(this.mView);
        if (tintList != null) {
            info2.mHasTintList = true;
            info2.mTintList = tintList;
        }
        Mode mode = ImageViewCompat.getImageTintMode(this.mView);
        if (mode != null) {
            info2.mHasTintMode = true;
            info2.mTintMode = mode;
        }
        if (!info2.mHasTintList && !info2.mHasTintMode) {
            return false;
        }
        AppCompatDrawableManager.tintDrawable(imageSource, info2, this.mView.getDrawableState());
        return true;
    }
}
