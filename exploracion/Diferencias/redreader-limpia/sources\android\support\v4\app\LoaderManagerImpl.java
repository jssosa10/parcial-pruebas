...
class LoaderManagerImpl extends LoaderManager {
    ...
    static class LoaderViewModel extends ViewModel {
        ...
        /* access modifiers changed from: 0000 */
        public void putLoader(int id, @NonNull LoaderInfo info2) {
            this.mLoaders.put(id, info2);
        }
        ...
        public void dump(String prefix, FileDescriptor fd, PrintWriter writer, String[] args) {
            if (this.mLoaders.size() > 0) {
                ...
                for (int i = 0; i < this.mLoaders.size(); i++) {
                    LoaderInfo info2 = (LoaderInfo) this.mLoaders.valueAt(i);
                    writer.print(prefix);
                    writer.print("  #");
                    writer.print(this.mLoaders.keyAt(i));
                    writer.print(": ");
                    writer.println(info2.toString());
                    info2.dump(innerPrefix, fd, writer, args);
                }
            }
        }
    }
    ...
    public <D> Loader<D> initLoader(int id, @Nullable Bundle args, @NonNull LoaderCallbacks<D> callback) {
        if (this.mLoaderViewModel.isCreatingLoader()) {
            ...
            LoaderInfo<D> info2 = this.mLoaderViewModel.getLoader(id);
            if (DEBUG) {
                String str = TAG;
                StringBuilder sb = new StringBuilder();
            ...
            return info2.setCallback(this.mLifecycleOwner, callback);
        } else {
    ...
    public <D> Loader<D> restartLoader(int id, @Nullable Bundle args, @NonNull LoaderCallbacks<D> callback) {
        if (this.mLoaderViewModel.isCreatingLoader()) {
            ...
            if (info2 != null) {
                priorLoader = info2.destroy(false);
            }
    ...
    public void destroyLoader(int id) {
        if (this.mLoaderViewModel.isCreatingLoader()) {
            ...
            LoaderInfo info2 = this.mLoaderViewModel.getLoader(id);
            if (info2 != null) {
                info2.destroy(true);
