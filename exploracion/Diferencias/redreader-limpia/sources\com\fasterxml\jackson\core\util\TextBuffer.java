...
public final class TextBuffer {
    ...
    public int contentsToWriter(Writer w) throws IOException {
        ...
        if (i >= 0) {
            ...
            if (len > 0) {
                w.write(this._inputBuffer, i, len);
            }
            return len;
        }
        int total = 0;
        ...
        if (arrayList != null) {
            ...
            for (int i2 = 0; i2 < end; i2++) {
                char[] curr = (char[]) this._segments.get(i2);
                int currLen = curr.length;
                w.write(curr, 0, currLen);
                total += currLen;
            }
        }
        int end2 = this._currentSize;
        if (end2 > 0) {
            w.write(this._currentSegment, 0, end2);
            total += end2;
        }
        return total;
    }
    ...
    public char[] expandCurrentSegment(int minSize) {
        ...
        char[] copyOf = Arrays.copyOf(curr, minSize);
        char[] curr2 = copyOf;
        this._currentSegment = copyOf;
        return curr2;
    }
