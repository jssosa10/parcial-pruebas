...
public final class TextViewCompat {
    ...
    private static class OreoCallback implements Callback {
        ...
        private void recomputeProcessTextMenuItems(Menu menu) {
            ...
            try {
                ...
                for (int i2 = 0; i2 < supportedActivities.size(); i2++) {
                    ResolveInfo info2 = (ResolveInfo) supportedActivities.get(i2);
                    menu.add(0, 0, i2 + 100, info2.loadLabel(packageManager)).setIntent(createProcessTextIntentForResolveInfo(info2, this.mTextView)).setShowAsAction(1);
                }
        ...
        private List<ResolveInfo> getSupportedActivities(Context context, PackageManager packageManager) {
            ...
            for (ResolveInfo info2 : packageManager.queryIntentActivities(createProcessTextIntent(), 0)) {
                if (isSupportedActivity(info2, context)) {
        ...
        private boolean isSupportedActivity(ResolveInfo info2, Context context) {
            boolean z = true;
            ...
            if (!(info2.activityInfo.permission == null || context.checkSelfPermission(info2.activityInfo.permission) == 0)) {
                z = false;
            }
            return z;
        }

        private Intent createProcessTextIntentForResolveInfo(ResolveInfo info2, TextView textView11) {
            return createProcessTextIntent().putExtra("android.intent.extra.PROCESS_TEXT_READONLY", !isEditable(textView11)).setClassName(info2.activityInfo.packageName, info2.activityInfo.name);
        }
