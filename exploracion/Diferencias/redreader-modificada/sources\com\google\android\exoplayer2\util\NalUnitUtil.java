...
public final class NalUnitUtil {
    ...
    public static boolean isNalUnitSei(String mimeType, byte nalUnitHeaderFirstByte) {
        if (MimeTypes.VIDEO_H264.equals(mimeType) && (nalUnitHeaderFirstByte & 31) == 6) {
            return true;
        }
    ...
    public static SpsData parseSpsNalUnit(byte[] nalData, int nalOffset, int nalLimit) {
        ...
        if (data.readBit() && data.readBit()) {
            ...
            int i4 = profileIdc;
            SpsData spsData = new SpsData(profileIdc, constraintsFlagsAndReservedZero2Bits, levelIdc, seqParameterSetId, cropUnitX, frameHeight, pixelWidthHeightRatio, separateColorPlaneFlag, frameMbsOnlyFlag, frameNumLength, picOrderCntType, picOrderCntLsbLength, deltaPicOrderAlwaysZeroFlag);
            return spsData;
        }
        pixelWidthHeightRatio = 1.0f;
        int i32 = chromaFormatIdc;
        int i42 = profileIdc;
        SpsData spsData2 = new SpsData(profileIdc, constraintsFlagsAndReservedZero2Bits, levelIdc, seqParameterSetId, cropUnitX, frameHeight, pixelWidthHeightRatio, separateColorPlaneFlag, frameMbsOnlyFlag, frameNumLength, picOrderCntType, picOrderCntLsbLength, deltaPicOrderAlwaysZeroFlag);
        return spsData2;
    }
    ...
    public static int findNalUnit(byte[] data, int startOffset, int endOffset, boolean[] prefixFlags) {
        ...
        if (prefixFlags != null) {
            ...
            boolean z2 = false;
            prefixFlags[0] = z2;
            prefixFlags[1] = length <= 1 ? !(!prefixFlags[2] || data[endOffset + -1] != 0) : data[endOffset + -2] == 0 && data[endOffset + -1] == 0;
