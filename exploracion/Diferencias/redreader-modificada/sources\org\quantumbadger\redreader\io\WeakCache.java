...
public final class WeakCache<K, V extends WritableObject<K>, F> implements CacheDataSource<K, V, F> {
    ...
    private final class CacheEntry {
        ...
        private CacheEntry(WeakReference<V> data2, WeakReferenceListManager<UpdatedVersionListener<K, V>> listeners2) {
            this.data = data2;
            this.listeners = listeners2;
        }

        private CacheEntry(WeakCache weakCache, WeakReference<V> data2) {
            this(data2, new WeakReferenceListManager<>());
        }
    }

    public WeakCache(CacheDataSource<K, V, F> cacheDataSource2) {
    ...
    public synchronized void performRequest(Collection<K> keys, TimestampBound timestampBound, RequestResponseHandler<HashMap<K, V>, F> handler) {
        ...
        for (K key : keys) {
            ...
            if (entry != null) {
                V value = (WritableObject) entry.data.get();
    ...
    public synchronized void performRequest(final K key, TimestampBound timestampBound, final RequestResponseHandler<V, F> handler, final UpdatedVersionListener<K, V> updatedVersionListener) {
        if (timestampBound != null) {
            ...
            if (existingEntry != null) {
                V existing = (WritableObject) existingEntry.data.get();
        ...
        this.cacheDataSource.performRequest(key, timestampBound, (RequestResponseHandler<V, F>) new RequestResponseHandler<V, F>() {
            ...
            public void onRequestSuccess(V result, long timeCached) {
                synchronized (WeakCache.this) {
                    WeakCache.this.put(result, false);
                    if (updatedVersionListener != null) {
                        ((CacheEntry) WeakCache.this.cached.get(key)).listeners.add(updatedVersionListener);
                    }
    ...
    public synchronized void put(V value, boolean writeDown) {
        ...
        if (oldEntry != null) {
            this.cached.put(value.getKey(), new CacheEntry(new WeakReference(value), oldEntry.listeners));
            oldEntry.listeners.map(this.updatedVersionListenerNotifier, value);
        } else {
            this.cached.put(value.getKey(), new CacheEntry(new WeakReference(value)));
        }
        if (writeDown) {
