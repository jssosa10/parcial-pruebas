...
public final class EGLSurfaceTexture implements OnFrameAvailableListener, Runnable {
    ...
    private static EGLSurface createEGLSurface(EGLDisplay display2, EGLConfig config, EGLContext context2, int secureMode) {
        ...
        if (secureMode == 1) {
            ...
            EGLSurface surface3 = EGL14.eglCreatePbufferSurface(display2, config, secureMode == 2 ? new int[]{12375, 1, 12374, 1, EGL_PROTECTED_CONTENT_EXT, 1, 12344} : new int[]{12375, 1, 12374, 1, 12344}, 0);
            if (surface3 != null) {
                surface2 = surface3;
            } else {
