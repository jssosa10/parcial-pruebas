...
final class DefaultOggSeeker implements OggSeeker {
        ...
        switch (this.state) {
            ...
            case 2:
                ...
                if (j == 0) {
                    ...
                    long position2 = getNextSeekPosition(j, input);
                    if (position2 >= 0) {
                        return position2;
                    }
                    ExtractorInput extractorInput = input;
                    position = skipToPageOfGranule(extractorInput, this.targetGranule, -(position2 + 2));
                }
                this.state = 3;
                return -(2 + position);
            case 3:
    ...
    public long getNextSeekPosition(long targetGranule2, ExtractorInput input) throws IOException, InterruptedException {
        ...
        if (granuleDistance < 0 || granuleDistance > 72000) {
            ...
            long j9 = granuleDistance;
            return Math.min(Math.max(position + (((j6 - j7) * granuleDistance) / (this.endGranule - this.startGranule)), j7), this.end - 1);
        }
        extractorInput.skipFully(pageSize);
