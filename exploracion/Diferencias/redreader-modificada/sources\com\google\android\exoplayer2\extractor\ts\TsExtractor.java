...
public final class TsExtractor implements Extractor {
        ...
        public void consume(ParsableByteArray sectionData) {
            ...
            if (sectionData.readUnsignedByte() == 2) {
                ...
                while (remainingEntriesLength > 0) {
                    ...
                    EsInfo esInfo = readEsInfo(parsableByteArray, esInfoLength);
                    if (streamType == 6) {
                ...
                for (int i6 = 0; i6 < trackIdCount; i6++) {
                    ...
                    TsPayloadReader reader2 = (TsPayloadReader) this.trackIdToReaderScratch.valueAt(i6);
                    if (reader2 != null) {
                        if (reader2 != TsExtractor.this.id3Reader) {
                            reader2.init(timestampAdjuster, TsExtractor.this.output, new TrackIdGenerator(programNumber, trackId2, 8192));
                        }
    ...
    public TsExtractor(int mode2, TimestampAdjuster timestampAdjuster, Factory payloadReaderFactory2) {
        this.payloadReaderFactory = (Factory) Assertions.checkNotNull(payloadReaderFactory2);
        this.mode = mode2;
        if (mode2 == 1 || mode2 == 2) {
    ...
    public boolean sniff(ExtractorInput input) throws IOException, InterruptedException {
        ...
        for (int startPosCandidate = 0; startPosCandidate < 188; startPosCandidate++) {
            ...
            while (true) {
                if (i >= 5) {
                    break;
                } else if (buffer[(i * TS_PACKET_SIZE) + startPosCandidate] != 71) {
    ...
    public void seek(long position, long timeUs) {
        ...
        for (int i = 0; i < timestampAdjustersCount; i++) {
            ...
            if ((timestampAdjuster.getTimestampOffsetUs() == C.TIME_UNSET) || !(timestampAdjuster.getTimestampOffsetUs() == 0 || timestampAdjuster.getFirstSampleTimestampUs() == j)) {
    ...
    public int read(ExtractorInput input, PositionHolder seekPosition) throws IOException, InterruptedException {
        ...
        if ((tsPacketHeader & 16) != 0) {
            payloadReader = (TsPayloadReader) this.tsPayloadReaders.get(pid);
        }
        ...
        int adaptationFieldLength = this.tracksEnded;
        ...
        if (this.mode != 2 && adaptationFieldLength == 0 && this.tracksEnded && inputLength != -1) {
    ...
    private void maybeOutputSeekMap(long inputLength) {
        if (!this.hasOutputSeekMap) {
            ...
            if (this.durationReader.getDurationUs() != C.TIME_UNSET) {
                TsBinarySearchSeeker tsBinarySearchSeeker2 = new TsBinarySearchSeeker(this.durationReader.getPcrTimestampAdjuster(), this.durationReader.getDurationUs(), inputLength, this.pcrPid);
