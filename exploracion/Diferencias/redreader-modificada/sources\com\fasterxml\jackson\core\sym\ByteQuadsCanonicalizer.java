...
public final class ByteQuadsCanonicalizer {
    ...
    private int _appendLongName(int[] quads, int qlen) {
        ...
        int[] iArr = this._hashArea;
        if (i > iArr.length) {
            this._hashArea = Arrays.copyOf(this._hashArea, this._hashArea.length + Math.max((start + qlen) - iArr.length, Math.min(4096, this._hashSize)));
        }
